...
contract STBToken is ERC20, Orchestrated {
    using SafeMath for uint256;

    bool public transferLimit;
    mapping(address => bool) public transferWhite;
    // split/10000
    uint public splitRate = 10000;


    constructor() ERC20("STB", "STB") {
        ERC20._mint(msg.sender, 500000 * 1e18);
        transferWhite[_msgSender()] = true;
        transferWhite[0x000000000000000000000000000000000000dEaD] = true;
        transferLimit = true;
        orchestrate(_msgSender(),0x7563c703);
        orchestrate(_msgSender(),0xdbceb005);
    }

    //0x7563c703
    function setSplitRate(uint value) public onlyOrchestrated("onlyOrchestrated: setSplitRate") {
        splitRate = value;
    }

    //0xdbceb005
    function split(uint rate) public onlyOrchestrated("onlyOrchestrated: split") {
        splitRate = splitRate.mul(rate).div(10000);
    }

    function setTransferLimit(bool value) public onlyOwner {
        transferLimit = value;
    }
    ...
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply.mul(splitRate).div(10000);
    }

    function balanceOf(address account) public view virtual override returns (uint256) {
        return _balances[account].mul(splitRate).div(10000);
    }
    ...
    function _transfer(
        ...
        require(!transferLimit || transferWhite[sender] || transferWhite[recipient] || address(0) == sender, "Token: transferLimit");

        uint balanceAmount = amount.mul(10000).div(splitRate);
        _beforeTokenTransfer(sender, recipient, balanceAmount);
        super._transfer(sender, recipient, balanceAmount);
    }
    ...
    function burn(uint256 amount) public virtual returns(bool) {
        ...
        emit Burn(_msgSender(), balanceAmount);
        return true;
    }
